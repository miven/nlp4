...
def train_epoch(model, training_data, optimizer, device, smoothing,opt):
    ...
    for  i,batch in enumerate(training_data):
        ...
        # forward
        optimizer.zero_grad()
        pred,atten_list = model(src_seq, src_pos, tgt_seq, tgt_pos)  # atten_list输出是6,batchsize*8 , tgt,src
# pred 经过了展评.方便下面计算loss而已.
        ...
        for i in check_dic:
            left=i
            right=check_dic[i]
            # left---------right对应的-atten加入loss中
            #从而atten越大越好.
            leftindex=src_word2idx1[left]
            rightindex=tgt_word2idx1[right]
            #是否出现.
# atten_list输出是6,batchsize*8 , tgt,src
        #src_seq         tgt_seq
...
def train(model, training_data, validation_data, optimizer, device, opt):
    ...
    for epoch_i in range(opt.epoch):
        ...
        train_loss, train_accu = train_epoch(
            model, training_data, optimizer, device, smoothing=opt.label_smoothing,opt=opt)
