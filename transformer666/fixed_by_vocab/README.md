...
train.py 是带词典的从0训练
 train2finetuen.py 是利用上面的权重做finetune训练.
...
2020-07-09,19点28
打算先复现transformer的翻译效果.
然后把权重赋值过来,做一些层上的修改.看效果. pycharm看代码先把项目的根目录设置成source root.
这样就保证了代码的跳转.



方法细节有2个,1个是注意力, 做查找, 找到了字典中的对,那么就把这个对的注意力. 和1做一个loss. 让学习尽量把 apple 的注意力都给苹果这个短语


这样学到最后就会自动匹配这些字典里面的对

第二个细节是, 用大量的拥有字典对的句子对,做finetune

这2点应该就能提升准去率.



这个项目是给予word的.如果以后变成wordpiece也同理
第一个word的所有piece 跟第二个word的所有piece 互相注意力对都要做loss.让他们变大.即可!!!!!!!!
