...
'''
跟峰哥做的项目,发现加载大数据时候有内存或者显存不够,无法一次性加载的问题.!
'''
...
def main():
    ...
    parser.add_argument('-save_data', required=True)  # 存好的东西是dict pickle化.
...
# 传入4个路径,加maxword参数加keepcase,2个字典. 传出4个 编码后的句子迭代器.
def  raw2index(train_src,train_tgt,valid_src,valid_tgt,max_word_seq_len,keep_case,src_word2idx,tgt_word2idx):
    ...
    return train_src_insts,train_tgt_insts,valid_src_insts,valid_tgt_insts
...
if __name__ == '__main__':
    main()
    '''
    python preprocess.py -train_src train_s.zh.atok -train_tgt train_s.en.atok -valid_src val.zh.atok -valid_tgt val.en.atok -save_data test.zh2en.atok.low.pt
    '''
