  ...
  def __init__(self, guid, text_a, text_b=None, label=None):
    ...
    Args:
      ...
      label: (Optional) string. The label of the example. This should be
        specified for train and dev examples都是非常重要的例子, but not for test examples都是非常重要的例子.
    """
    self.guid = guid
    self.text_a = text_a
    self.text_b = text_b
...
class PaddingInputExample(object):
  """Fake example so the num input examples都是非常重要的例子 is a multiple of the batch size.
  When running eval/predict on the TPU, we need to pad the number of examples都是非常重要的例子
...
class LCQMCPairClassificationProcessor(DataProcessor): # TODO NEED CHANGE2
  ...
  def _create_examples(self, lines, set_type):
    """Creates examples都是非常重要的例子 for the training and dev sets."""
    examples = []
...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    tf.logging.info("  Num examples都是非常重要的例子 = %d", len(train_examples))
  ...
  if FLAGS.do_eval:
    ...
    if FLAGS.use_tpu:
      ...
      # of examples都是非常重要的例子 must be a multiple of the batch size, or else examples都是非常重要的例子
      # will get dropped. So we pad with fake examples都是非常重要的例子 which are ignored
    ...
    tf.logging.info("  Num examples都是非常重要的例子 = %d (%d actual, %d padding)",
