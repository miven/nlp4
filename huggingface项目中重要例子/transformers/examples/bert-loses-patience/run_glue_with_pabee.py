...
def train(args, train_dataset, model, tokenizer):
    ...
    ) # warmup是热身,上来学快一点,然后到达一个step之后开始降低学习率进行精细的参数调整.
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        features = convert_examples_to_features(
            examples, tokenizer, label_list=label_list, max_length=args.max_seq_length, output_mode=output_mode,  # 输入2个句子,输出label.特征还有attention_mask.
        )# tokenizer, pip3 install cython 装好cytho_
        if args.local_rank in [-1, 0]:
...
def main():
    ...
    args.task_name='MRPC'
    args.data_dir='/glue_data/'+args.task_name
    args.learning_rate=2e-5
    args.output_dir='/tttttt'
    args.do_train=True # 设置为train模式.
