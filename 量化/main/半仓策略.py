...
class mid_class():
    def __init__(self, this_exchange):
        '''  https://www.fmz.com/api  这里面可以查到所有函数的说明.
        ...
        self.jyd = self.exchange.GetCurrency()  #exchange.GetCurrency()，返回交易所操作的货币对名称如LTC_BTC，传统期货CTP返回的固定为STOCK。返回值:string类型。
    ...
    def get_depth(self):
        ...
        try:
            ...
            self.Ask = self.Depth['Asks']  # 询问价格
            self.Bids = self.Depth['Bids']  # 成交价格.
            return True
        except:
            return False

    def get_ohlc_data(self, period=PERIOD_M5):  # 输入一个时间间隔,返回这个间隔时候的K线.
        '''
        获取K线信息
    ...
    def refreash_data(self):
        ...
        Returns:
            刷新信息成功返回 'refreash_data_finish!' 否则返回相应刷新失败的信息提示,这个函数每一次交易之后调用一下,保证信息是最新的.
        '''
...
class juncang_class():
    def __init__(self, mid_class):
        ...
        self.last_trade_price = self.jys.last   # 79行的最后成交价格.代表股票当前的价格.


# 获取所有需要的信息.吧他们邪道对象的属性道中.
    def make_need_account_info(self):
        self.jys.refreash_data() # 先刷新保证最新性.
        ...
        self.need_buy = (self.half_money - self.B * now_price) / now_price #其实就是把总金额的一般换成股票
    ...
    def do_juncang(self):
        if self.need_buy > 0.001:# 因为交易所手续费是千分之一.
    ...
    def if_need_trade(self, condition, prama):
        ...
        if condition == 'price':  # 上下波动超过xxx,我们就做均仓.
            if abs(self.jys.last - self.last_trade_price) / self.last_trade_price > prama:
                self.do_juncang()
                self.last_trade_price = self.jys.last # 保存当时价格,给下次均仓做铺垫.


# %%
# 这个也不用动.  网页上的代码会自动调用这个main
def main():
    test_mid = mid_class(exchange)  # 跑的时候会从回测开始的时间开始跑.
    Log(test_mid.refreash_data())
    test_juncang = juncang_class(test_mid)

    while (True):
# 每一次等待一段时间,来模拟真实的时间流逝.
            Sleep(60)
            test_juncang.make_need_account_info()  # 就是均仓算法里面的获取最新信息
            test_juncang.if_need_trade('price', 0.02)
