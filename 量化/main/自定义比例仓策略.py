...
'''
class juncang_class():
    def __init__(self, mid_class,rate):
        self.rate=rate # 这个比例表示我要把仓里面总金额的多少比例干成股票
        self.jys = mid_class
...
# 获取所有需要的信息.吧他们邪道对象的属性道中.
    def make_need_account_info(self):
        ...
        self.gupiao_money = self.total_money *self.rate
        self.need_buy = (self.gupiao_money - self.B * now_price) / now_price #其实就是把总金额的一般换成股票
        # 需要买进的数量  ,当然只有价格比之前下降了,我们这个才是正数.否则是一个负数.

# 卖掉x s.t.     ( self.B-x)*now_price+self.money

        self.need_sell = (self.B*now_price-self.gupiao_money) / now_price
        # 需要卖掉的股票的数量.
...
def main():
    ...
    test_juncang = juncang_class(test_mid,0.75)
