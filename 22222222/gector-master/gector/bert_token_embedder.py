...
class BertEmbedder(TokenEmbedder):
    """ # 一个单词输出一个768? 应该是的, 里面用了很多平均化的方法,这个类的forward需要仔细看看.
    ...
    def forward(
        ...
        """
        Parameters         # 这段就是对于wordpiece 和 token之间区别行的处理.这里面的token指的就是我们的一个英文单词.
        ----------
        input_ids : ``torch.LongTensor``
        ...
        offsets : ``torch.LongTensor``, optional
            ...
            of the first wordpiece for each token. # offset 表示toekn里面最后一个piece所在的坐标,正是下面例子中的3,4
            For example, if you had the sentence "Definitely not", and if the corresponding
            wordpieces were ["Def", "##in", "##ite", "##ly", "not"], then the input_ids
            would be 5 wordpiece ids, and the "last wordpiece" offsets would be [3, 4].
            If offsets are provided, the returned tensor will contain only the wordpiece # 然后我们只取这个结尾坐标的768.所以我们一个token只有一个768向量.所以这里面用的是offset技术,没用cls技术.cls可能还是太粗糙了.
...
class PretrainedBertEmbedder(BertEmbedder): # 这个类用来复用bert预训练数据.
    ...
    def __init__(
        ...
        if special_tokens_fix: #xlnet 不用修改
            try:
