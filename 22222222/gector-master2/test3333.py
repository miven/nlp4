...
def main(args):
    ...
    else: # 生成字典. 利用数据集生成字典. 就是一个简单的计数器.
    ...
    print("Model is dumped","训练全部结束,model存在了args.model_dir, +'model.th'")
...
if __name__ == '__main__':
    ...
    args.model_dir='tmpModel2' # 存储model 的路径.
    ...
    args.n_epoch=5 # 必须大于cold,否则存储的参数数量不够.
    main(args)
  ...
  (tag_labels_projection_layer): TimeDistributed(
    (_module): Linear(in_features=768, out_features=5002, bias=True)
  )
  (tag_detect_projection_layer): TimeDistributed(
    (_module): Linear(in_features=768, out_features=4, bias=True)
  )
) 这个是predict 里面的参数, 而train 里面是28!!!!!!!!!!!!!!!!!!这个需要看看why

核心是vocab的构造. 他里面决定了 num_classes. 而这个东西又由output.txt的生成代码决定.
所以核心又在output_vocabulary/labels.txt里面
这个labels.txt生成代码在utils/preprocess_data.py
