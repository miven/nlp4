...
parser.add_argument('--control_code', type=str, required=False,
                                        help='control code to use for this file. must be in the vocabulary, else it will error out.')
parser.add_argument('--sequence_len', type=int, required=False,
...
args.text_file='moby_dick.txt'
args.control_code= 'Moby'
...
# 输入就是一个单预料就行.一堆句子就行. read结果是一个大字符串表示整个篇章
...
# if you want to run non-english text, please tokenize separately using ./fast applybpe and then run this script on the .bpe file with utf8 encoding              # 只过滤出英文字符. apply 就是encode算法.

tokenized_train_text = re.findall(r'\S+|\n', tokenized_train_text) # 把非空白和\n都找出来,之间用空格分隔. 也就是等价于split(' ')
...
with tf.io.TFRecordWriter(tfrecords_fname) as writer:
    for i in tqdm.tqdm(range(0, len(tokenized_train_text), seq_length)): # 每255个单词算一个句子.
        flag_input, inputs = numericalize(domain+tokenized_train_text[i:i+seq_length])  # 注意输入要牵头加上domain.
        flag_output, outputs = numericalize(tokenized_train_text[i:i+seq_length+1])  # ctrl算法输入是 i:j 输出是i:j+1 ,所以一个句子总共是
        total += 1  # flag_input flag_output 是来判断是不是有错误. 所以下面如果其中一个有错误我们就跳过他.然后continue
