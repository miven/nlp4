...
def main():
    ...
    parser.add_argument('--user-dir',
                        help='path to fairseq examples都是非常重要的例子/translation_moe/src directory')
    ...
    if args.user_dir is None:
        args.user_dir = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),  # examples都是非常重要的例子/
            'translation_moe',
            'src',
        )
        if os.path.exists(args.user_dir):
            ...
            raise RuntimeError(
                'cannot find fairseq examples都是非常重要的例子/translation_moe/src '
