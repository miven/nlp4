...
import pandas as pd
df=pd.read_sql(sql="select * from ent_basic_info where  id >=90854",con=conn)
...
df.to_sql('ent_basic_info_copy0917',engine,if_exists='append', index=False)
print("11111111")
...
if 1:
    ...
    path='全国企业搜索.xlsx'
    now=pd.read_excel(path,sheet_name='绿色化工')
    print(now['企业名称'])
    all1=now['企业名称']
    qiye=all1
    import requests
...
out=df[df['ent_name'].isin(all1)].drop_duplicates('ent_name',keep='last')
print(out)
out.to_csv('\data\dfasdfa.csv')
##
