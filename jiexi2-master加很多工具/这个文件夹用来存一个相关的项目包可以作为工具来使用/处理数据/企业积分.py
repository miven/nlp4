...
engine = create_engine('mysql+pymysql://aioc:i8NnFvh6t@mysql-cn-north-1-4fda8775bfcc4f32.public.jcloud.com:3306/bi_yuxi'
...
# 创建连接
conn = pymysql.connect(host='mysql-cn-north-1-4fda8775bfcc4f32.public.jcloud.com', port=3306, user='aioc',
                       passwd='i8NnFvh6t', db='bi_yuxi')


import pandas as pd
df=pd.read_sql(sql="select ent_name from ent_basic_info where id>90852",con=conn)
print(df)
...
if 1:
    for ii in range(len(qiye)):
        ...
        tmp = requests.post('http://yuxi-cloud.xjoycity.com/bi/ent/entscoreQuery?queryString=' + str(i))
        print(ii)
    print('over')
