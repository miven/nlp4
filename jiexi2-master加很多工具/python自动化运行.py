...
'''
因为shell无法读取python的配置文件,所以用python对shell自动化运行脚本进行控制封装
https://blog.csdn.net/bible_reader/article/details/82430371

把shell需要的参数用python穿进去.
'''
...
tmp2 = config['ports']['ports']


tmp = tmp.split(' ')
# ports = [i for i in tmp] #ports是一个类变量.

# os.system("sh shell自动化启动.sh "+"\""+tmp2+"\"") #注意如果不写sh命令 会报permission deny
...
#数组需要用""抱起来.
# os.system("./shell自动化启动.sh "+tmp2)
print(command)
import subprocess

with open('tmpshell.sh','w') as  f:
    f .write(command)

# subprocess.run(command)

os.system("sh tmpshell.sh "+"\""+tmp2+"\"") #注意如果不写sh命令 会报permission deny

#利用字符串读写文件,来实现shell 跨平台运行.
