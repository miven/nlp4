...
'''
这里面是核心函数!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
这里面定义了集中transforemer 的不同大小的结构. 服务器越好久用越大的模型即可.
'''
# 只需要注册这个大模型.即可.他里面调用的函数不用再注册了. 在大模型里面调用即可. 这就是fairseq方便的地方.
...
# 这种archi表示的是参数设置函数,第一个传入要设置的model名字, 第二个是arch 名字也就是当前系数设置起一个名字方便调用.
