...
'''
这个代码把unpaired sentence 变成 MaskedLanguagePairDataset
首先他集成平行预料这个类,

LDA是主题模型, 只能提取关键词,如果给一篇文章得到他的摘要,只能用这种nlg算法.
'''
class MaskedLanguagePairDataset(LanguagePairDataset):
    ...
    def collate(self, samples, pad_idx, eos_idx, left_pad_source=True, left_pad_target=False):
        ...
        '''
        1.Returns the total number of elements in the input tensor.
返回元素数目 （torch.numel）
        '''
    ...
    def replace(self, x):
        ...
        probs = torch.multinomial(self.pred_probs, len(x), replacement=True) # replacement=True 表示有放回的抽样, 这样抽的更平均.
