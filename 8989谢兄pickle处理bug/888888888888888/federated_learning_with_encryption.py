...
def get_data(n_clients):
    ...
    X = np.c_[X, np.ones(X.shape[0])]   # 只是在原始数据上加一个新column,从而wx+b 就可以用wx来替代了.这个代码是线性拟合bosdun房价.
#np.c_是按行连接两个矩阵，就是把两矩阵左右相加，要求行数相等。
    # The features are already preprocessed
    # Shuffle
    perm = np.random.permutation(X.shape[0])
    X, y = X[perm, :], y[perm] # 数据做shuffle
...
def decrypt_vector(private_key, x):
    return np.array([private_key.decrypt(i) for i in x])  # 解密算法由private_key提供.
...
class Server:
    ...
    def decrypt_aggregate(self, input_model, n_clients):
        return decrypt_vector(self.privkey, input_model) / n_clients  # 服务器端利用decrypt_vector对数据进行解密. 除以clients数量是因为 保证服务器对于更新权重的归一化.  服务器端,求和后再进行解密.这样就保证了.不会看到单独的用户信息.
...
class Client:
    ...
    def compute_gradient(self):
        ...
        """
        delta = self.predict(self.X) - self.y #   (wx-y)**2 对于w的导数是  2(wx-y)*x
    ...
    def encrypted_gradient(self, sum_to=None):
        ...
        """
        gradient = self.compute_gradient()
        encrypted_gradient = encrypt_vector(self.pubkey, gradient) # 把得到的梯度进行加密
...
def federated_learning(X, y, X_test, y_test, config):
    ...
    server = Server(key_length=config['key_length']) # 创建服务器,在服务器里面设置好 公钥私钥对.
    ...
    for i in range(n_iter):
        ...
        encrypt_aggr = clients[0].encrypted_gradient(sum_to=None)  # 加密算法在public_key中
        for c in clients[1:]:
        ...
        # Take gradient steps
        for c in clients:  #每一轮服务器更新好参数后,再重新发送给client.
...
def local_learning(X, y, X_test, y_test, config):
    ...
    for c in clients:
        c.fit(config['n_iter'], config['eta']) # 独立学习的train 和predict部分.
        y_pred = c.predict(X_test)
        mse = mean_square_error(y_pred, y_test)
        print('{:s}:\t{:.2f}'.format(c.name, mse))
