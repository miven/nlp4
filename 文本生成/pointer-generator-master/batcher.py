...
class Batch(object):
  """Class representing a minibatch of train/val/test examples都是非常重要的例子 for text summarization."""
...
class Batcher(object):
  """A class to generate minibatches of data. Buckets examples都是非常重要的例子 together based on length of the encoder sequence."""
  ...
  def __init__(self, data_path, vocab, hps, single_pass):
    ...
    if single_pass:
      ...
      self._num_batch_q_threads = 1  # just one thread to batch examples都是非常重要的例子
      self._bucketing_cache_size = 1 # only load one batch's worth of examples都是非常重要的例子 before bucketing; this essentially means no bucketing
      self._finished_reading = False # this will tell us when we're finished reading the dataset
    else:
      self._num_example_q_threads = 16 # num threads to fill example queue
      self._num_batch_q_threads = 4  # num threads to fill batch queue
      self._bucketing_cache_size = 100 # how many batches-worth of examples都是非常重要的例子 to load into cache before bucketing
  ...
  def fill_example_queue(self):
    ...
    while True:
      ...
      except StopIteration: # if there are no more examples都是非常重要的例子:
