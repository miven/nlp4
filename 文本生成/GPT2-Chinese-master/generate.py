...
'''
这份代码是使用cpu来predict的代码.我们直接使用cpu版本即可.
'''
def is_word(word):
...
def fast_sample_sequence(model, context, length, temperature=1.0, top_k=30, top_p=0.0, device='cpu'):
    ...
    if len(context) > 1:
        _, past = model(inputs[:, :-1], None)[:2]   # _是预测出来的东西, 输入当前seq,和past.输出后一个字的预测.
        prev = inputs[:, -1].view(1, -1)  # prev 是text的最后一个字
    else:
        past = None
        prev = inputs
    generate = [] + context
    with torch.no_grad():
        for i in trange(length): # length 表示要生成的句子长度.
            output = model(prev, past=past)
            output, past = output[:2]
            output = output[-1].squeeze(0) / temperature
            filtered_logits = top_k_top_p_filtering(output, top_k=top_k, top_p=top_p) # topk算法,把排名不靠前的都用-inf替换.   下面是多项式抽样. 取1个.
            next_token = torch.multinomial(torch.softmax(filtered_logits, dim=-1), num_samples=1)
            generate.append(next_token.item())
            prev = next_token.view(1, 1)  # 把新出来的放prev里面.
    return generate
...
def generate(n_ctx, model, context, length, tokenizer, temperature=1, top_k=0, top_p=0.0, repitition_penalty=1.0, device='cpu',
    ...
    if is_fast_pattern: # 推断的2种模式,一个是正常的单字,一个是fast模式
...
def main():
    ...
    args.length=50
    args.nsamples=4
    args.prefix='萧炎'        # 从哪个词开始写.
    args.fast_pattern=True        # 从哪个词开始写.
    args.save_samples=True        # 从哪个词开始写.
    args.save_samples_path='/output'        # 从哪个词开始写.
    args.model_path='/999'        # 从哪个词开始写.
    args.segment=True        # 这个中文时候应该为True, 表示以词为单位,
    args.model_config='/999/config.json'        # 这个中文时候应该为True, 表示以词为单位,
    args.tokenizer_path='/999/vocab.txt'        # 这个中文时候应该为True, 表示以词为单位,
...
#从词典加载tokenizer, 里面有2个token
    tokenizer = tokenization_bert.BertTokenizer(vocab_file=args.tokenizer_path)
    model = GPT2LMHeadModel.from_pretrained(args.model_path) # 传入文件夹,之后读取好魔性.
    model.to(device)
    model.eval()
