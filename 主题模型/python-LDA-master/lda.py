...
class LDAModel(object):
    ...
    def __init__(self,dpre):
        ...
        # nw,词word在主题topic上的分布         # 主题和单词作为特征的数量
        # nwsum,每各topic的词的总数
        # nd,每个doc中各个topic的词的总数      # doc和主题作为特征的数量.
        # ndsum,每各doc中词的总数
...
# 参数初始化都是0.
    ...
    def sampling(self,i,j):
        ...
        topic = self.Z[i][j]  # 获取当前位置所属于的topic分类.
        word = self.dpre.docs[i].words[j]  # 下行是去除掉当前主题分类的影响.
        ...
        self.p = (self.nw[word] + self.beta)/(self.nwsum + Vbeta) * \
                 (self.nd[i] + self.alpha) / (self.ndsum[i] + Kalpha)  # 调整概率
        for k in range(1,self.K):
            self.p[k] += self.p[k-1]

        u = random.uniform(0,self.p[self.K-1])  # 重新抽样.
        for topic in range(self.K):
            if self.p[topic]>u:  # 找到新的topic
                break
# 主题变回去即可.
        self.nw[word][topic] +=1
        self.nwsum[topic] +=1
        ...
        return topic        # est是estimate 运行.
    def est(self):
        # Consolelogger.info(u"迭代次数为%s 次" % self.iter_times)
        for x in range(self.iter_times):  # 迭代次数
            for i in range(self.dpre.docs_count):   # 文章个数
                for j in range(self.dpre.docs[i].length):  # 文章特征数.
...
输出最核心的就是这个. 加个argmax就得到了所有的文章分类.
'''
