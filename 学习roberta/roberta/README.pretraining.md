...
Data should be preprocessed following the [language modeling format](/examples都是非常重要的例子/language_model), i.e. each document should be separated by an empty line (only useful with `--sample-break-mode complete_doc`). Lines will be concatenated as a 1D text stream during training.
